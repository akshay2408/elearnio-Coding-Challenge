...
RSpec.describe "Courses", type: :request do
  ...
  describe "PUT courses#update" do
    ...
    it "should update the bio" do
      ...
      put "/courses/#{courses.first.id}", :params => new_course_params.to_json, :headers => { "Content-Type": "application/json" }
      json = JSON.parse(response.body)
  ...
  describe "DELETE courses#destroy" do
    ...
    it 'removes course from table' do
      delete "/courses/#{courses.first.id}", :params => params.to_json, :headers => { "Content-Type": "application/json" }
      json = JSON.parse(response.body)
